...
class StdGP:
	...
	def __init__(self, operators=[("+",2),("-",2),("*",2),("/",2)], max_initial_depth = 6, population_size = 500, 
		...
		double=False, Sf=4, Sp=3, Switch=False):
	...
	def fit(self, Tr_x, Tr_y, Te_x = None, Te_y = None):
		if self.verbose:
			...
			if self.double == True:
				print(color.GREEN + "    > Double:             "+str(self.double) + color.END)
				print(color.GREEN + "    > Sf:                 "+str(self.Sf) + color.END)
				print(color.GREEN + "    > Sp:                 "+str(self.Sp) + color.END)
				print(color.GREEN + "    > Switch:             "+str(self.Switch) + color.END)
			print()
	...
	def nextGeneration(self):
		...
		while len(newPopulation) < self.population_size:
			offspring = getOffspring(self.rng, self.population, self.tournament_size, self.double, self.Sf, self.Sp, self.Switch)
